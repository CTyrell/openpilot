...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    if self.use_nn:
      ...
      self.nnff_alpha_up_down = [0.0, 0.0] # for increasing/decreasing magnitude of lat accel/jerk
  ...
  def update(self, active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk, lat_plan=None, model_data=None):
    ...
    if not active:
      ...
      lookahead = interp(CS.vEgo, [10.0, 30.0], [0.3, 1.2]) # seconds
      ...
      error_rate=(lookahead_lateral_jerk - actual_lateral_jerk) if self.use_steering_angle else 0.0
